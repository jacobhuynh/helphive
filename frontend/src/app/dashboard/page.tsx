...
const Button = ({ className, variant = "default", size = "default", ...props }: ButtonProps) => {
    ...
    const sizeStyles = size === "sm" ? "h-9 px-3" : size === "lg" ? "h-11 px-8" : "h-10 px-4";
...
export default function Dashboard() {
    const [reccommendedOpportunities, setReccommendedOpportunities] = useState<Opportunity[]>([]);
    ...
    async function userInterested(id: string) {
        ...
        try {
            await fetch(`http://127.0.0.1:8000/add_volunteering/${encodedEmail}?volunteer_id=${id}`, {
                method: "POST",
